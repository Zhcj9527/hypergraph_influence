<template>
    <div class='pattern-comparator2' ref='root'>
    </div>
</template>
...
export default{
    ...
    methods:{
        draw_pattern(data){
            ...
            var svg = d3.select('.pattern-comparator2').append('svg')
    ...
    mounted(){
        ...
        // 默认值
        this.$axios.post('get_patcomptor_data2', "agr_3")
            .then((res) => {
                d3.select('.pattern-comparator2').selectAll('*').remove();  // 移除原来的画布所画内容
                var data = res.data
                console.log("pattern_data2", data);
                this.$bus.$emit("default_Alg_B", data)
                this.draw_pattern(data)
            })
            .catch((error) => {
                console.log('get_patcomptor_data2', error);
            })

        // 事件触发后->改变算法
        this.$bus.$on('algorithm-selected-hot-dbl', (d) => {
            console.log(d)
            this.$axios.post('get_patcomptor_data2', d.algorithm)
            .then((res) => {
                d3.select('.pattern-comparator2').selectAll('*').remove();  // 移除原来的画布所画内容
                var data = res.data
                console.log("pattern_data", data);
                this.$bus.$emit("dblclick_Alg_B", data)
                this.draw_pattern(data)
                // this.draw(data);
            })
            .catch((error) => {
        ...
        this.$bus.$on('algorithm-selected-pat2', (d) => {
            console.log(d)
            this.$axios.post('get_patcomptor_data2', d.algorithm)
            .then((res) => {
                d3.select('.pattern-comparator2').selectAll('*').remove();  // 移除原来的画布所画内容
                var data = res.data
                console.log("pattern_data", data);
                this.$bus.$emit("selected-pat2_Alg_B", data)
                this.draw_pattern(data)
                // this.draw(data);
            })
            .catch((error) => {
